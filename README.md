...
- 형상은 모두 폴리곤. 클릭으로 선택, 정점 드래그, 선 더블클릭으로 정점 추가, Delete 로 정점/형상 삭제.
  펜(P) 도구로 점을 찍어 폴리곤을 그리고, 사각형(R) 도구로 드래그해 사각형 추가. 0.5 mm 스냅, r < 0 불가.
...
모델 두 가지: 원뿔 파라미터(꼭짓점 높이·바닥 반경)와 **자유 프로파일**(반경 0, 8, 16, 24, 33.5 mm 에서의 높이 5개).
프로파일 모델은 원뿔뿐 아니라 바깥으로 올라가는 접시형, 상단이 평평한 절두 원뿔, 혹 모양까지 탐색한다.
